...
private:
    ...
    int deleteNode(TreeNode*& root, size_t index, std::string s, std::vector<std::string>& keyword, TreeNode*& eow)
    {
        ...
        if (s[index + 1] == '\0') // at the end of the string
        {
            ...
            if (root->data != s[index])
            {
                while (root->data != s[index])
                {
                    if (root->data > s[index]) root = root->left;
        ...
        if (s[index + 1] != '\0') // still in the string
        {
            ...
            if (s[index] < root->data)  deleteNode(root->left, index, s, keyword, eow);
            else
            if (s[index] > root->data)  deleteNode(root->right, index, s, keyword, eow);
            else
    ...
    void addHistorytoFile(std::string keyword)
    {
        ...
        std::time_t t = std::time(0);   // get time now
            ...
            fout << (now->tm_year + 1900) << '/' << (now->tm_mon + 1) << '/' <<  now->tm_mday << " " << keyword << "\n";
        fout.close();
    }
